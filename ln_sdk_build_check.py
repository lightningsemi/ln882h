...
class BuildCheck:
    ...
    def check_gcc_proj(self, proj_path_list):
        ...
        for proj in proj_path_list:
            ...
            ######  config  ######
            cmd = "{_cmake}  -DUSER_PROJECT={_up}  -S .  -B  {_b}  -G  Ninja".format(_cmake=self.cmake_filepath, _up=proj, _b=build_path)

            try:
                # result = subprocess.check_call(cmd, stdout=subprocess.DEVNULL)
                ...
                print("==========" * 10)
                print("Error: cmake config gcc proj {_p} failed: {_e}".format(_p=proj, _e=err))
                print("==========" * 10)
                return False

            ######  build  ######
            cmd = "{_cmake}  --build  {_b}  --target all".format(_cmake=self.cmake_filepath, _b=build_path)
            try:
                result = subprocess.check_call(cmd)
                if 0 == result:
                ...
                print("==========" * 10)
                print("Error: cmake build gcc proj {_p} failed: {_e}".format(_p=proj, _e=err))
                print("==========" * 10)
                return False
    ...
    def __do_keil_proj_build(self, keil_proj_filepath, keil_proj_name):
        ...
        cmd = " ".join([self.uv4_filepath, "-b", keil_proj_filepath, "-o", buildout_filepath])
        try:
            result = subprocess.check_call(cmd, stderr=subprocess.DEVNULL)
            if 0 != result:
                print("==========" * 10)
    ...
    def __find_cmake(self, path):
        ...
        result_path = os.path.join(path, "cmake" + exe_suffix)
        if os.path.exists(result_path):
            retcode = True
            cmake_filepath = result_path
...
def main():
    ...
    keil_proj_list = [
        ...
        "project/bootload/bootram_ln/keil_ln882h",
