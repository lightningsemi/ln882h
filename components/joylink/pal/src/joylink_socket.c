...
int32_t jl_platform_gethostbyname(char *hostname, char *ip_buff, uint32_t buff_len)
{
    ...
    void *sin_addr = NULL;
    char host[]="baidu.com";
    jl_platform_printf("socket:jl_platform_gethostbyname \r\n");
    ...
    *********************************/
    ...
    if (ret != 0) {
        jl_platform_printf("socket:getaddrinfo() failed, ret = %d\r\n", ret);
        return -1;
    }
    ...
    for (cur = res; cur != NULL; cur = cur->ai_next)
    {
        ...
        jl_platform_printf("socket:family = %s, socktype = %s, protocol = %s\r\n", family, socktype, protocol);
        jl_platform_printf("socket:ai_flags = %d, ai_family = %d, ai_socktype = %d, ai_protocol = %d, ai_addrlen = %d\r\n", 
            cur->ai_flags, 
            cur->ai_family, 
            cur->ai_socktype, 
            cur->ai_protocol, 
	    ...
	    jl_platform_printf("socket:[%s] ip address:[%s]\r\n", hostname, ip_buff);
        if(ip != NULL)
            break;
	...
	if(pGethost != NULL)
	{
		...
		for(; *pptr != NULL; pptr++){
			...
			jl_platform_printf("socket:[%s] ip address:[%s] [%lu]\r\n", hostname, inet_ntoa(addrConv), addrConv.s_addr);
       ...
       jl_platform_memset(ip_buff,  0, buff_len);
        jl_platform_strncpy(ip_buff, inet_ntoa(inaddr), buff_len);
         jl_platform_printf("socket:[%s] ip address\r\n", ip_buff);
        return 0;
...
{
#if 1//def __LINUX_PAL__
    int32_t ret_domain = -1;
    jl_platform_printf("socket:jl_platform_get_socket_proto_domain \r\n");
...
int32_t jl_platform_socket(JL_SOCK_PROTO_DOMAIN_T domain, JL_SOCK_PROTO_TYPE_T type, JL_SOCK_PROTO_PROTO_T protocol)
{
    ...
    jl_platform_printf("socket:jl_platform_socket \r\n");
    ...
    socket_num++;
    soc =socket(in_domain, in_type, in_protocol);
    jl_platform_printf("socket:jl_platform_socket = %d \r\n",soc); 
    return soc;
#else
    return -1;
#endif
...
int32_t jl_platform_connect(int32_t s, const jl_sockaddr *name, uint32_t namelen)
{
    ...
    jl_platform_printf("socket:jl_platform_connect socket = %d,sa = %d,ip:%d,%d,%d,%d,\r\n",s,addr.sa_family,addr.sa_data[2],addr.sa_data[3],addr.sa_data[4],addr.sa_data[5]);
    if(addr.sa_data[2] == 0 && addr.sa_data[3] == 0 && addr.sa_data[4] == 0 && addr.sa_data[5] == 0)
    {
        //TODO:
    }
    ret = connect(s, &addr, namelen);
    jl_platform_printf("socket:jl_platform_connect ret = %d,%d\r\n",ret ,errno);
    return ret;
#else
    return -1;
#endif
...
#if 1//def __LINUX_PAL__
    int32_t flags;
    jl_platform_printf("socket:jl_platform_fcntl \r\n");
...
{
#if 1//def __LINUX_PAL__
    jl_platform_printf("socket:jl_platform_send \r\n");
...
{
#if 1//defined (__LINUX_PAL__) || defined (__ESP_32_PAL__)
    int32_t ret;
    fd_set fds;
    struct timeval tv;

    //jl_platform_printf("socket:jl_platform_recv \r\n");
    FD_ZERO(&fds);
    FD_SET(fd, &fds);
    ...
    if (ret < 0)
    {
        jl_platform_printf("socket:[%s][%d] : ret = %s\r\n", __FUNCTION__, __LINE__, strerror(errno));
    }
    else if (ret == 0)
    {
        // 超时返回-1，防止跟断开连接无法区分。
        ret = -1;
        jl_platform_printf("socket:[%s][%d] : timeout\r\n", __FUNCTION__, __LINE__);
    }
    else
    {
        ...
        {
            // 返回值为0表示连接以断开
            jl_platform_printf("socket:[%s][%d] : ret = %d, errno = %s\r\n", __FUNCTION__, __LINE__, ret, strerror(errno));
        }
    }
...
int32_t jl_platform_select(int32_t maxfdp1, jl_fd_set *readset, jl_fd_set *writeset, jl_fd_set *exceptset,  jl_timeval *timeout)
{
    ...
    //jl_platform_printf("socket:jl_platform_select \r\n");
...
char* jl_platform_inet_ntoa(jl_in_addr addr)
{
    ...
    inet_ntoa_r(tmp_addr,buf,sizeof(buf));
