...
joylink_dev_parse_ctrl(const char *pMsg, user_dev_status_t *userDev)
{
	...
	if(NULL != pStreams){
		...
		for( iCnt = 0; iCnt < iSize; iCnt++){
			...
			if(!jl_platform_strcmp(LN_USER_DATA_POWER, pSId->valuestring)){
			    ...
			    joylink_dev_user_data_set( LN_USER_DATA_POWER,userDev);
			}
            #if 0 
