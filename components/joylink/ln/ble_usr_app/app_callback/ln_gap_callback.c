        ...
        ke_task_id_t const src_id)
{
    ...
    ln_app_gatt_exc_mtu(0);
    struct ln_gapc_set_le_pkt_size_cmd pkt_size;
    pkt_size.tx_octets = 251;
    pkt_size.tx_time   = 2120;
    OS_MsDelay(1000);
    ln_app_param_set_pkt_size(conidx, &pkt_size);
    
    struct ln_gapc_conn_param conn_param;
    conn_param.intv_min = 20;  // 10x1.25ms  (7.5ms--4s)
    conn_param.intv_max = 40;  // 10x1.25ms  (7.5ms--4s)
    conn_param.latency  = 10;
    conn_param.time_out = 3000;  //ms*n
    ln_app_update_param(conidx, &conn_param);
    ln_set_ble_con(conidx);
    ln_set_ble_connect_flag(0);
