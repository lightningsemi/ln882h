...
#if defined(TLS_SAVE_TICKET)
    // NOTE: not supported to delete KV by prefix.
...
unsigned int fakeRandom()
{
    return ((unsigned)HAL_UptimeMs() % 65536 + 1);
...
static int _ssl_random(void *p_rng, unsigned char *output, size_t output_len)
{
    ...
    while (rnglen > 0) {
        *(output + rngoffset) = (unsigned char)fakeRandom() ;
        rngoffset++;
        rnglen--;
    }
    return 0;
                              ...
                              const char *client_pwd, size_t client_pwd_len)
{
    ...
    if (NULL == saved_session) {
        do {
            ...
            if (ret != 0 || len == 0) {
                HAL_Printf("get kv ticket ret=%d", ret);
