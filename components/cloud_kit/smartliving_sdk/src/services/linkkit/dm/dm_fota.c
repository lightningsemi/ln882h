...
int dm_fota_perform_sync(_OU_ char *output, _IN_ int output_len)
{
    ...
    while (1) {
        ...
        if (IOT_OTA_IsFetchFinish(ota_handle)) {
            ...
            if (file_isvalid == 0) {
                dm_log_debug("IOT_OTAG_CHECK_FIRMWARE: invalid !!!");
                HAL_Firmware_Persistence_Stop();
                IOT_OTA_ReportProgress(ota_handle, IOT_OTAP_CHECK_FALIED, NULL);
                ctx->is_report_new_config = 0;
                return FAIL_RETURN;
            } else {
                dm_log_debug("IOT_OTAG_CHECK_FIRMWARE: valid !!!");
                // TODO: LN OTA status check.
                extern int  ota_part_verify(void);
                extern void ota_reboot_start(void);

                if (0 == ota_part_verify()) {
                    ota_reboot_start();
                }
