...
static OS_Thread_t host_task_h;
...
nimble_port_freertos_init(TaskFunction_t host_task_fn)
{
     ...
     */
//    xTaskCreate(host_task_fn, "ble", configMINIMAL_STACK_SIZE + 800,
//                NULL, tskIDLE_PRIORITY + 4, &host_task_h);
    
     if(OS_OK != OS_ThreadCreate(&host_task_h, "host", host_task_fn, NULL, OS_PRIORITY_NORMAL, 4096)) {
        LOG(LOG_LVL_ERROR, "host task create fail!\r\n");
		return;
    }
}
...
{
    if (host_task_h.handle) {
        vTaskDelete(host_task_h.handle);
    }
}
