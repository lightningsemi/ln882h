 ...
 * </pre>
 */
#define EM_BLE_TXISOBUFSETUP_ADDR   (0x40430000 + EM_BLE_TX_ISO_BUF_OFFSET)
#define EM_BLE_TXISOBUFSETUP_INDEX  0x00000000
#define EM_BLE_TXISOBUFSETUP_RESET  0x00000000

__STATIC_INLINE__ uint16_t em_ble_txisobufsetup_get(int elt_idx)
{
    return EM_BLE_RD(EM_BLE_TXISOBUFSETUP_ADDR + elt_idx * REG_EM_BLE_TX_ISO_BUF_SIZE);
}

__STATIC_INLINE__ void em_ble_txisobufsetup_set(int elt_idx, uint16_t value)
{
...
__STATIC_INLINE__ void em_ble_txisobufsetup_pack(int elt_idx, uint8_t txisomute, uint8_t txisolength)
{
...
__STATIC_INLINE__ void em_ble_txisobufsetup_unpack(int elt_idx, uint8_t* txisomute, uint8_t* txisolength)
{
...
__STATIC_INLINE__ uint8_t em_ble_txisobufsetup_txisomute_getf(int elt_idx)
{
    uint16_t localVal = EM_BLE_RD(EM_BLE_TXISOBUFSETUP_ADDR + elt_idx * REG_EM_BLE_TX_ISO_BUF_SIZE);
    return ((localVal & ((uint16_t)0x00000100)) >> 8);
}

__STATIC_INLINE__ void em_ble_txisobufsetup_txisomute_setf(int elt_idx, uint8_t txisomute)
{
    ASSERT_ERR((((uint16_t)txisomute << 8) & ~((uint16_t)0x00000100)) == 0);
...
__STATIC_INLINE__ uint8_t em_ble_txisobufsetup_txisolength_getf(int elt_idx)
{
    uint16_t localVal = EM_BLE_RD(EM_BLE_TXISOBUFSETUP_ADDR + elt_idx * REG_EM_BLE_TX_ISO_BUF_SIZE);
    return ((localVal & ((uint16_t)0x000000FF)) >> 0);
}

__STATIC_INLINE__ void em_ble_txisobufsetup_txisolength_setf(int elt_idx, uint8_t txisolength)
{
    ASSERT_ERR((((uint16_t)txisolength << 0) & ~((uint16_t)0x000000FF)) == 0);
 ...
 * </pre>
 */
#define EM_BLE_TXISODATABUF_ADDR   (0x40430002 + EM_BLE_TX_ISO_BUF_OFFSET)
...
__STATIC_INLINE__ uint16_t em_ble_txisodatabuf_get(int elt_idx, int reg_idx)
{
    ASSERT_ERR(reg_idx <= 32);
    return EM_BLE_RD(EM_BLE_TXISODATABUF_ADDR + elt_idx * REG_EM_BLE_TX_ISO_BUF_SIZE + reg_idx * 2);
}

__STATIC_INLINE__ void em_ble_txisodatabuf_set(int elt_idx, int reg_idx, uint16_t value)
{
    ASSERT_ERR(reg_idx <= 32);
...
__STATIC_INLINE__ uint16_t em_ble_txisodatabuf_getf(int elt_idx, int reg_idx)
{
    ASSERT_ERR(reg_idx <= 32);
...
__STATIC_INLINE__ void em_ble_txisodatabuf_setf(int elt_idx, int reg_idx, uint16_t txisodatabuf)
{
    ASSERT_ERR(reg_idx <= 32);
