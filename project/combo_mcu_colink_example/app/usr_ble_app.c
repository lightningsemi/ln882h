...
#define DATA_TRANS_1ST_RX_UUID      {0xfb,0x34,0x9b,0x5f,0x80,0x00,0x00,0x80,0x00,0x10,0x00,0x00,0xb0,0xbb,0x00,0x00} //28be4a4a-cd67-11e9-a32f-2a2ae2dbcce4
#define DATA_TRANS_1ST_TX_UUID      {0xfb,0x34,0x9b,0x5f,0x80,0x00,0x00,0x80,0x00,0x10,0x00,0x00,0xb1,0xbb,0x00,0x00}//28be4cb6-cd67-11e9-a32f-2a2ae2dbcce4
#else
...
#if (_LN882H_SDK_V1_8_)
static void colink_ble_set_adv_data(void)
{
    ...
    uint16_t name_len = strlen(get_colink_xcrypt_handle()->colinkBleName);
    ...
    if(name_len)
    {
        ...
        adv_data_len += 2;
        memcpy(&adv_data[2], get_colink_xcrypt_handle()->colinkBleName, name_len);
...
static void ln_ble_disconnect_cb(void *arg)
{
    ...
    if (!ln_kv_has_key((const char *)DEVICE_CONFIG_PARA))
    {
        Log_w("ln kv can not find DEVICE_CONFIG_PARA");
        get_colink_mcu_handle()->reset();
    }
...
#if (_LN882H_SDK_V1_8_)
    hexdump(LOG_LVL_INFO, "[COLINK_RECV_DATA]", (void *)p_gatt_write->value, p_gatt_write->length);
#else
    hexdump(LOG_LVL_INFO, "[recv data]", (void *)p_gatt_write->value, p_gatt_write->length);
#endif

#if (_LN882H_SDK_V1_8_)
    get_colink_ble_handle()->refresh(p_gatt_write->conidx, p_gatt_write->handle);
    get_colink_ble_handle()->receive(p_gatt_write->value, p_gatt_write->length);
#endif
/*
...
static void ln_ble_app_init(void)
{
    ...
    {
#if (BLE_DATA_TRANS_SERVER)
        ln_ble_trans_svr_add(&g_user_svc_desc_tab[0].desc);
    ...
    while(1)
    {
        if(OS_OK == usr_queue_msg_recv((void *)&usr_msg, OS_WAIT_FOREVER))
        {
            switch(usr_msg.id)
            {
                ...
                {
                    ln_trans_svr_send_t *svc_send_cmd = (ln_trans_svr_send_t *)(usr_msg.msg);
                    hexdump(LOG_LVL_INFO, "[COLINK_SEND_DATA]", (void *)svc_send_cmd->data, svc_send_cmd->len);
                    Log_i("notify conidx = %d , handle = %d\r\n", svc_send_cmd->conn_idx, svc_send_cmd->hdl);
                    ln_ble_trans_svr_ntf(svc_send_cmd);
                    blib_free(svc_send_cmd->data);
                    svc_send_cmd->data = NULL;
                }
                break;
