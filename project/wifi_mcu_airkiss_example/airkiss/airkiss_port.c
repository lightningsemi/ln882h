...
{
	sniffer_cfg_t sniCfg = {0};
	sniCfg.enable = LN_TRUE;
	sniCfg.type = 0;
	sniCfg.filter_mask = WIFI_SNIFFER_FILTER_MASK_DATA;
	wifi_sta_set_sniffer_cfg(&sniCfg, (wifi_sniffer_cb_t)callback);
}

static void ak_stop_sniffer_callback(uint8_t *frame, uint32_t len)
{
    ak_port_printf("stop sniffer success!\r\n");
}

void ak_port_wifi_sniffer_disable(void)
{
	sniffer_cfg_t sniCfg = {0};
	if (0 != wifi_sta_set_sniffer_cfg(&sniCfg, (wifi_sniffer_cb_t)ak_stop_sniffer_callback))
	{
		ak_port_printf("%s() stop sniffer fail!\n", __func__);
	}
}
