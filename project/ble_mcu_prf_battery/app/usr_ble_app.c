...
void user_timer_callback (void *arg)
{
    ...
    static int level = 0;
    batt_svr_level_upd_req_t req = {0};
    req.batt_level = level;
    req.bas_instance = 0;
    ln_batt_svr_send_lvl(0, &req);
    level++;
    if(level > 100)
        level = 0;
}
...
static void ln_ble_connect_cb(void *arg)
{
    ...
    {
        ln_batt_cli_discover_server(conn_idx);
    }
#endif
}
...
static void ln_ble_disconnect_cb(void *arg)
{
    ...
    ble_evt_disconnected_t *data = blib_malloc(sizeof(ble_evt_disconnected_t));
    memcpy(data, evt_disconn, sizeof(ble_evt_disconnected_t));
    usr_queue_msg_send(BLE_MSG_DISCONN_IND, sizeof(ble_evt_disconnected_t), data);
...
static void ln_ble_gatt_write_req_cb(void *arg)
{
    ...
    uint8_t handle_idx = p_gatt_write->handle - batt_server.start_hdl;
    LOG(LOG_LVL_TRACE, "ln_ble_gatt_write conn_id=%d,handle=%d",p_gatt_write->conidx, handle_idx);

    if(BATT_DECL_LVL_CHAR_CCC == handle_idx)
    {
        uint8_t *bat_ntf_cfg = blib_malloc(sizeof(uint8_t));
        *bat_ntf_cfg = p_gatt_write->value[0];
        usr_queue_msg_send(BLE_MSG_BATT_LVL_NTF_CFG, sizeof(uint8_t), bat_ntf_cfg);
    }
}
    ...
    while(1)
    {
        if(OS_OK == usr_queue_msg_recv((void *)&usr_msg, OS_WAIT_FOREVER))
        {
            switch(usr_msg.id)
            {
                ...
                case BLE_MSG_BATT_LVL_NTF_CFG:
                {
                    uint8_t bat_ntf_cfg = *((uint8_t *)usr_msg.msg);
                    LOG(LOG_LVL_TRACE,"BLE_MSG_CLIENT_CHAR_CONFIG %d\r\n",bat_ntf_cfg);
                    if(PRF_CLI_START_NTF == bat_ntf_cfg)
                    {
                        user_timer_start(&ble_usr_timer);
                    }
                    else if(PRF_CLI_STOP_NTFIND == bat_ntf_cfg)
                    {
                        user_timer_stop(&ble_usr_timer);
                    }
