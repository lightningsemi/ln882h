...
/**
    I2C使用说明：
                ...
                2. AT24C04、OLED、SHT30、MPU9250是通过I2C挂载在一起的,I2C的SCL和SDA要接上拉电阻。接线如下：
                                                                ...
                                                                ln8620              OLED                AT24C04             SHT30               MPU9250            BMP280
...
void ln_i2c_init()
{
    hal_gpio_afio_select(GPIOB_BASE,GPIO_PIN_8,I2C0_SCL);      //初始化I2C引脚
    hal_gpio_afio_select(GPIOB_BASE,GPIO_PIN_9,I2C0_SDA);
    hal_gpio_afio_en(GPIOB_BASE,GPIO_PIN_8,HAL_ENABLE);        //使能功能复用
...
int hal_i2c_master_7bit_write(uint32_t base, uint8_t dev_addr, const uint8_t * buf, uint16_t buf_len)
{
    ...
    //1. check busy    
    while (1 == i2c_busy_getf(base)) { if (timeout++ > TIMEOUT_CYCLE) { i2c_swrst_setf(base, 1); i2c_swrst_setf(base, 0); return -1; } }
    ...
    //5. wait addr send ok
    timeout = 0;
    while (0 == i2c_addr_getf(base)) { if (timeout++ > TIMEOUT_CYCLE) { return -1; } }
    ...
    for (uint16_t i = 0; i < buf_len; i++)
    {
        //wait tx empty flag
        timeout = 0;
        while (0 == i2c_txe_getf(base)) { if (timeout++ > TIMEOUT_CYCLE) { return -1; } }
    ...
    while (0 == i2c_btf_getf(base)) { if (timeout++ > TIMEOUT_CYCLE) { return -1; } }
...
int hal_i2c_master_7bit_read_target_address(uint32_t base, uint8_t dev_addr,uint8_t *target_addr,uint8_t target_addr_len,uint8_t * buf, uint16_t buf_len)
{
    ...
    for(int i = buf_len;i > 0; i --)
    {
        ...
        if (buf_len == 1) 
        {
            ...
            //wait tx empty flag
            timeout = 0;
            while (0 == i2c_rxne_getf(base)) {if (timeout++ > TIMEOUT_CYCLE) { return -1; } }
            //read data
            *buf = i2c_dr_get(base);
        }
        else
...
void hal_i2c_address_scan(uint32_t base)
{
    ...
    for(dev_addr = 0;dev_addr <= 254;dev_addr = dev_addr + 2 )
    {
        timeout = 0;
        //check busy    
        while (1 == i2c_busy_getf(base)) { if (timeout++ > TIMEOUT_CYCLE) { i2c_swrst_setf(base, 1); i2c_swrst_setf(base, 0); break; } }
        ...
        //wait addr send ok
        timeout = 0;
        while (0 == i2c_addr_getf(base)) { if (timeout++ > TIMEOUT_CYCLE) { break; } }
        if (timeout++ > TIMEOUT_CYCLE)  continue;
...
int hal_24c04_read(uint16_t word_addr, uint8_t * buf, uint16_t buf_len)
{
   ...
   if(word_addr + buf_len >= 256)
   {
       ...
       {
            i = word_addr - 256;
            if (0 != (ret = hal_i2c_master_7bit_read_target_address(I2C_BASE, AT24C04_ADDR_W | 0x02,&i, 1,buf , buf_len)))
            {
                return ret;
            }
       }
       ...
       {
            i = word_addr;
            if (0 != (ret = hal_i2c_master_7bit_read_target_address(I2C_BASE, AT24C04_ADDR_W,&i, 1,buf,256 - word_addr)))
            {
                return ret;
            }
            i = 0;
            if (0 != (ret = hal_i2c_master_7bit_read_target_address(I2C_BASE, AT24C04_ADDR_W | 0x02,&i,1, buf + 256 - word_addr,buf_len - (256 - word_addr))))
            {
                return ret;
            }
       }
   ...
   {
       i = word_addr;
       if (0 != (ret = hal_i2c_master_7bit_read_target_address(I2C_BASE, AT24C04_ADDR_W,&i, 1,buf,buf_len)))
       {
           return ret;
       }
   }
...
static int hal_24c04_write_byte(uint16_t word_addr, uint8_t data)
{
    ...
    if (0 != (ret = hal_i2c_master_7bit_write_target_address(I2C_BASE, drv_addr,buf, 1 ,buf + 1, 1))) {
        return ret;
    }
...
void ln_i2c_test()
{
    ...
    {   
        ...
        pressure_read_sensor_data();                                       //更新压力计信息
