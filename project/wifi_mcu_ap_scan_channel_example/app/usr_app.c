...
#define AP_SCAN_TASK_STACK_SIZE   10*256 //Byte
static OS_Thread_t g_ap_scan_thread;

static void ap_scan_task_entry(void *params)
{
	app_ap_scan();
}
...
void creat_usr_app_task(void)
{
    ...
    if(OS_OK != OS_ThreadCreate(&ble_g_usr_app_thread, "BleUsrAPP", ble_app_task_entry, NULL, OS_PRIORITY_BELOW_NORMAL, BLE_USR_APP_TASK_STACK_SIZE)) 
    {
		...
		if(OS_OK != OS_ThreadCreate(&g_ap_scan_thread, "AP_SCAN", ap_scan_task_entry, NULL, OS_PRIORITY_BELOW_NORMAL, AP_SCAN_TASK_STACK_SIZE)) 
    {
        LN_ASSERT(1);
    }
