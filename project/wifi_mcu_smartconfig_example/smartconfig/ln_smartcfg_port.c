...
static void sc_stop_sniffer_callback(uint8_t *frame, uint32_t len)
{
    sc_log("stop sniffer success!\r\n");
}

void ln_sc_wifi_sniffer_disable(void)
{
	sniffer_cfg_t sniCfg = {0};
	if (0 != wifi_sta_set_sniffer_cfg(&sniCfg, (wifi_sniffer_cb_t)sc_stop_sniffer_callback))
	{
		sc_log("%s() stop sniffer fail!\n", __func__);
	}
}
