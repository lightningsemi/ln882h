...
class BeforeBuild:
    __DEFAULT_FLASH_TABLE_NAME = "flash_partition_table.h"
    __ALIGN = "32"
    __IMAGE_HEADER_SIZE_STR = "0x100"
    ...
    def prepare(self, *args):
        ...
        if (self.headfile_filepath is None) or (len(self.headfile_filepath) == 0):
            self.headfile_filepath = os.path.join(os.path.dirname(self.partcfg_filepath),
                                                BeforeBuild.__DEFAULT_FLASH_TABLE_NAME)
        return True
        ...
        with open(self.headfile_filepath, "w+") as f:
            ...
            try:
                for i in value:
                    ...
                    temp1 = "".join(["#define ",  format(name1," <"+ BeforeBuild.__ALIGN),  " (",  i['start_addr'],  ")"])
                    temp2 = "".join(["#define ",  format(name2," <"+ BeforeBuild.__ALIGN),  " (1024*",  str(i['size_KB']),  ')'])
            ...
            temp = "".join(["\n\n#define ",  format("IMAGE_HEADER_SIZE"," <"+ BeforeBuild.__ALIGN),  " ",  "("+ BeforeBuild.__IMAGE_HEADER_SIZE_STR+")"])
            f.writelines(temp)
            endcommont = "\n\n#endif /* __" + file_v + "__ */\n\n"
